...
record := TestRecord{Value: sql.NullString{Valid: false}}
if !record.Value.Valid {
    // Field was NULL in database
}

record = TestRecord{Value: sql.NullString{String: "test", Valid: true}}
if record.Value.Valid && record.Value.String == "test" {
    // Field had a value in database
}
```

## Files Modified

- [v1/db/row_adapter.go](v1/db/row_adapter.go): Added `setSQLNullField()` and `isSQLNullType()` helper functions, updated `setFieldFromValue()` to handle null types

## Related Issues

See CODE_REVIEW.md for more context and other enhancement opportunities.
